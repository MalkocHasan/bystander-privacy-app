...
export const AdminNotification: React.FC = () => {
    const { pendingRequests, approveRequest, denyRequest, currentUserRole } = useHomeStore();
                    ...
                    {activeRequests.map((req) => (
                        <div key={req.id} className="p-4 bg-white">
                            <p className="text-sm text-slate-600 mb-3">
                                ...
                                <span className="font-bold text-teal-600 mx-1">
                                    {req.requestType === 'prayer' ? 'Prayer Mode' : 'Privacy'}
                                </span>
                                for
                                <span className="font-bold text-slate-800 ml-1">{req.deviceName}</span>.
                            </p>
